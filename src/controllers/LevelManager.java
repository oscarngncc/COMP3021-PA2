...
public class LevelManager {
     ...
     * <p> The {@link String} backing the property may be {@code null} if no level is loaded from the filesystem.
     */
    @NotNull
    ...
    private void loadLevelNamesFromDisk() {
        // TODO wip -- missing loadLevelNames
        try {
            //Stream<Path> walk = Files.walk(this.mapDirectory, 0, FileVisitOption.FOLLOW_LINKS );
            ...
            if ( mapDirectory.toFile().exists() ) {
                ...
                for (final File fileEntry : mapDirectory.toFile().listFiles()) {
                    if (!fileEntry.isDirectory()) {
                        ...
                        int end = fileEntry.toString().lastIndexOf('.');
                        if ( end > 0 ){
                            if ( fileEntry.toString().substring(end+1).equals("map") ){
        ...
        catch ( Exception e ){ System.out.println("Error in loadLevelNamesFromDisk\n"); e.printStackTrace();  }
    ...
    public void setLevel(@Nullable String levelName) throws IllegalArgumentException {
        // TODO - wip?
        if (levelName.isBlank() || levelName == null || ! levelNames.contains(levelName) )
            throw new IllegalArgumentException("Level Name is Blank!");
        else {
            curLevelNameProperty.setValue(levelName);
            System.out.println("The Level, " + levelName + ", is set!");
        }
    }
